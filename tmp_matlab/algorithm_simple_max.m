...
%run("soundstage2.m"); 
iY=[mY(1:4:end), mY(2:4:end), mY(3:4:end), mY(4:4:end)];
nY=iY;
len = length(iY(:,1));
close all
...
for i=1:len
    ...
    if phase < 2
        out(i) = (phase-1)*nY(i,2) + (2-phase)*nY(i,1);
    elseif phase < 3
        out(i) = (phase-2)*nY(i,3) + (3-phase)*nY(i,2);
    else
        out(i) = (phase-3)*nY(i,4) + (4-phase)*nY(i,3);
    end
    phaseA(i) = phase;
end
%%
